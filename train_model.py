...
# # Define your target variable (shifted to predict the next day's close)
# data['Target'] = data['Prev Close'].shift(-1)  # Shift by -1 for future prediction

# # Drop any rows with NaN in the target column after shifting
# data.dropna(subset=['Target'], inplace=True)

# # Select features (drop the target column and any non-numeric columns)
# X = data.drop(columns=['Prev Close', 'Target', 'Date', 'Symbol', 'Series'], errors='ignore')
# y = data['Target']
...
# Define your target variable (e.g., predicting the next day's close)
data['Target'] = data['Close'].shift(-1)  # Shift by -1 for future prediction

# Drop any rows with NaN in the target column after shifting
data.dropna(subset=['Target'], inplace=True)

# Select features (keeping only the relevant columns)
X = data[['Open', 'High', 'Low', 'Close', 'Volume']]  # Use relevant features
y = data['Target']
